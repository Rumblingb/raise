...
// Session cache: a successful LLM board is persisted; an identical brief replays
// Gemma's own earlier output instantly, labeled as cached — honest and demo-fast.
import { mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { createHash } from 'node:crypto';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
const CACHE_DIR = join(dirname(fileURLToPath(import.meta.url)), '..', 'workspace', 'cache');
const cachePath = brief => join(CACHE_DIR, createHash('sha256').update(brief.trim().toLowerCase()).digest('hex').slice(0, 16) + '.json');

export async function interpret(brief, { timeoutMs = 120000 } = {}) {
  if (process.env.MUSEFRAME_BRAIN === 'local') return heuristicBoard(brief);
  try {
    const cached = JSON.parse(readFileSync(cachePath(brief), 'utf8'));
    cached.brain = `${MODEL} (local — cached from this session's earlier run)`;
    return cached;
  ...
  try {
    ...
    try { mkdirSync(CACHE_DIR, { recursive: true }); writeFileSync(cachePath(brief), JSON.stringify(board)); } catch { /* cache is best-effort */ }
    return board;
  } catch (e) {
...
// Warm Ollama so the first demo brief doesn't pay the cold-load tax (~90s on this machine).
export async function warmBrain({ timeoutMs = 180000 } = {}) {
  if (process.env.MUSEFRAME_BRAIN === 'local') return { warmed: false, reason: 'MUSEFRAME_BRAIN=local' };
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
    const res = await fetch(`${OLLAMA}/api/generate`, {
    ...
    return { warmed: false, reason: e.name === 'AbortError' ? 'timeout' : String(e.message) };
  } finally {
    clearTimeout(timer);
  }
}
