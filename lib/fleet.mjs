...
function cursorAuth() {
  if (!existsSync(CURSOR_BIN)) return { installed: false, logged_in: false };
  try {
    const out = execFileSync(CURSOR_BIN, ['status'], { encoding: 'utf8', timeout: 8000 }).trim();
    const logged_in = !/not logged in/i.test(out);
    return { installed: true, logged_in, detail: out.split('\n')[0] || out };
  } catch (e) {
    const msg = String(e.stdout || e.message || e);
    return { installed: true, logged_in: !/not logged in/i.test(msg), detail: msg.slice(0, 120) };
  }
}
...
export function fleetStatus() {
  ...
  const workers = WORKERS.map(w => {
    ...
    if (w.kind === 'external') {
      ...
      if (!auth.installed) {
        presence = 'absent';
        note = 'Cursor is not installed on this machine — the adapter is designed and blocked on founder install. Shown truthfully, not faked.';
      } else if (!auth.logged_in) {
        presence = 'auth required';
        note = 'cursor-agent installed but not logged in — run `cursor-agent login` once (founder-only). Jobs fail with Authentication required until then.';
      } else {
        presence = 'ready';
        note = 'Headless cursor-agent available — dispatch runs inside the generated artifact directory.';
      }
    } else {
